 ...
 * @brief This constructor is mandatory for Swig because it can be used with no
 * parameters.
 * @param log  Flag to print logs or not
 */
PMMLlib::PMMLlib(bool log)
    : _log(log), _pmmlFile(""), _doc(NULL), _rootNode(NULL), _currentNode(NULL),
      _nbModels(0), _currentModelName(""), _currentModelType(kUNDEFINED) {
    SetRootNode();
}
 ...
 */
void PMMLlib::SetCurrentModel(std::string modelName, PMMLType type) {
...
/**
 * Set the current model and its type.
 * @brief Throw an exception if there is no model or more than one model with
 * name "modelName" in the PMML file
 * @param modelName Name of the model to load (ie content of 'modelName'
 * attribute)
 */
void PMMLlib::SetCurrentModel(std::string modelName) {
    ...
    if (nC != 1) {
        ...
        oss << nC;
        string msg = "SetCurrentModel(modelName) : found " + oss.str() +
...
/**
 * Set the current model and its type.
 * @brief Throw an exception if no model is found or if there are more than one
 * model in the PMLL file
 */
void PMMLlib::SetCurrentModel() {
    ...
    if (nC != 1) {
        std::ostringstream oss;
        oss << nC;
        string msg = "SetCurrentModel() : found " + oss.str() +
                     " model(s) in PMML file.\n";
        msg += "Use SetCurrentModel(modelName) or "
               "SetCurrentModel(modelName,type).";
        throw msg;
    }
 ...
 */
void PMMLlib::SetHeader(std::string copyright, std::string description,
                        std::string appName, std::string appVersion,
                        std::string annotation) {
    xmlNodePtr headerNode =
        xmlNewChild(_rootNode, 0, (const xmlChar *)"Header", 0);
    xmlNewProp(headerNode, (const xmlChar *)"copyright",
               (const xmlChar *)(copyright.c_str()));
    xmlNewProp(headerNode, (const xmlChar *)"description",
               (const xmlChar *)(description.c_str()));

    xmlNodePtr appNode =
        xmlNewChild(headerNode, 0, (const xmlChar *)"Application", 0);
    xmlNewProp(appNode, (const xmlChar *)"name",
               (const xmlChar *)(appName.c_str()));
    xmlNewProp(appNode, (const xmlChar *)"version",
               (const xmlChar *)(appVersion.c_str()));

    xmlNewChild(headerNode, 0, (const xmlChar *)"Annotation",
 ...
 */
void PMMLlib::AddMiningSchema(std::string name, std::string usageType) {
    ...
    if (!miningSchemaNode) {
        miningSchemaNode =
            xmlNewChild(netNode, 0, (const xmlChar *)"MiningSchema", 0);
    }

    // then append the node
    xmlNodePtr miningFieldNode =
        xmlNewChild(miningSchemaNode, 0, (const xmlChar *)"MiningField", 0);
    xmlNewProp(miningFieldNode, (const xmlChar *)"name",
               (const xmlChar *)(name.c_str()));
    xmlNewProp(miningFieldNode, (const xmlChar *)"usageType",
 ...
 * @return Number of models
 */
int PMMLlib::GetModelsNb() { return _nbModels; }
 ...
 * @return Pointer to the node found
 */
xmlNodePtr PMMLlib::GetPtr(int index, std::string name) {
    xmlNodePtr node = NULL;
...
void PMMLlib::Write(std::string file) {
    ...
    if (ret == -1) {
        std::string msg("  *** Error :: unable to write the PMML file \"" +
                        file + "\"");
        cout << msg << endl;
        throw msg;
    }
 ...
 */
void PMMLlib::ExportCpp(std::string file, std::string functionName,
                        std::string header) {
 ...
 * @param header Header of the function
 */
void PMMLlib::ExportFortran(std::string file, std::string functionName,
                            std::string header) {
    if (_currentModelType == kANN)
 ...
 * @param header Header of the function
 */
void PMMLlib::ExportPython(std::string file, std::string functionName,
                           std::string header) {
    if (_currentModelType == kANN)
 ...
 */
std::string PMMLlib::ExportPyStr(std::string functionName, std::string header) {
    if (_currentModelType == kANN)
...
std::string PMMLlib::ReadNetworkStructure() {
    ...
    if (node_layer != NULL) {
        ...
        while (node_layer != NULL &&
               ...
               (string((const char *)(node_layer->next->name)) !=
                "NeuralOutputs")) {
    ...
    // Output layers
    xmlNodePtr node_outputs =
        GetChildByName(_currentModelNode, "NeuralOutputs");
        ...
        if (node_output != NULL) {
            while (node_output != NULL) {
                ...
                if (child != NULL) {
                    xmlNodePtr fieldName = child->children; // NormContinuous
                    if (fieldName != NULL) {
                        if (string((const char *)(fieldName->name)) ==
                            "NormContinuous")
                            structure += "@";
...
int PMMLlib::GetNormalizationType() {
    ...
    if (string((const char *)(node_field->name)) == "NormContinuous") {
        ...
        if (dnorm1 * dnorm2 < -0.5) { // case of kMinusOneOne
            return 0;
        } else { // case of kCR, kZeroOne
            return 1;
        }
    }
 ...
 */
void PMMLlib::GetNormalisationInput(int index, double *dnorm) {
    CheckNeuralNetwork();
    dnorm[0] = 0.0;
    dnorm[1] = 0.0;
    xmlNodePtr node_inputs = GetChildByName(_currentModelNode, "NeuralInputs");
 ...
 * @param[out] dnorm Array that contains the mean and the standard deviation
 */
void PMMLlib::GetNormalisationOutput(int index, double *dnorm) {
    CheckNeuralNetwork();
    dnorm[0] = 0.0;
    dnorm[1] = 0.0;
 ...
 */
int PMMLlib::GetNbLayers() { return (GetNbHiddenLayers() + 2); }
 ...
 */
double PMMLlib::GetNeuronBias(int layer_index, int neu_index) {
    CheckNeuralNetwork();
 ...
 */
double PMMLlib::GetPrecNeuronSynapse(int layer_index, int neu_index,
                                     int prec_index) {
    CheckNeuralNetwork();
 ...
 */
// LCOV_EXCL_START
void PMMLlib::SetNeuralNetName(int index, std::string name) {
    CheckNeuralNetwork();
    ...
    if (_doc != NULL) {
        ...
        xmlNewProp(node_ann, (const xmlChar *)"modelName",
                   (const xmlChar *)(name.c_str()));
    }
 ...
 */
void PMMLlib::AddDataField(std::string fieldName, std::string displayName,
                           std::string optype, std::string dataType,
                           std::string closure, double leftMargin,
                           double rightMargin, bool interval) {
    // if 'DataDictionary' node does not exist, create it
    xmlNodePtr dataDictNode = GetChildByName(_rootNode, "DataDictionary");
    if (!dataDictNode) {
        dataDictNode =
            xmlNewChild(_rootNode, 0, (const xmlChar *)"DataDictionary", 0);
    }

    // then append the node
    xmlNodePtr dataFieldNode =
        xmlNewChild(dataDictNode, 0, (const xmlChar *)"DataField", 0);
    xmlNewProp(dataFieldNode, (const xmlChar *)"name",
               (const xmlChar *)(fieldName.c_str()));
    xmlNewProp(dataFieldNode, (const xmlChar *)"displayName",
               (const xmlChar *)(displayName.c_str()));
    xmlNewProp(dataFieldNode, (const xmlChar *)"optype",
               (const xmlChar *)(optype.c_str()));
    xmlNewProp(dataFieldNode, (const xmlChar *)"dataType",
               (const xmlChar *)(dataType.c_str()));

    if (interval) {
        xmlNodePtr intervalNode =
            xmlNewChild(dataFieldNode, 0, (const xmlChar *)"Interval", 0);
        xmlNewProp(intervalNode, (const xmlChar *)"closure",
                   (const xmlChar *)(closure.c_str()));
        stringstream ss;
        ss << scientific << leftMargin;
        xmlNewProp(intervalNode, (const xmlChar *)"leftMargin",
                   (const xmlChar *)(ss.str().c_str()));
        ss.str("");
        ss << scientific << rightMargin;
        xmlNewProp(intervalNode, (const xmlChar *)"rightMargin",
                   (const xmlChar *)(ss.str().c_str()));
    }
}
...
void PMMLlib::AddNeuralNetwork(std::string modelName,
    ...
    xmlNodePtr netNode =
        xmlNewChild(_rootNode, 0, (const xmlChar *)"NeuralNetwork", 0);
    xmlNewProp(netNode, (const xmlChar *)"modelName",
               (const xmlChar *)(_currentModelName.c_str()));
    xmlNewProp(netNode, (const xmlChar *)"functionName",
               (const xmlChar *)(function.c_str()));
    xmlNewProp(netNode, (const xmlChar *)"numberOfLayers",
               (const xmlChar *)"0");
...
void PMMLlib::AddNeuralInput(int id, std::string inputName, std::string optype,
    ...
    if (!neuralInputsNode) {
        neuralInputsNode =
            xmlNewChild(netNode, 0, (const xmlChar *)"NeuralInputs", 0);
        xmlNewProp(neuralInputsNode, (const xmlChar *)"numberOfInputs",
                   (const xmlChar *)"0");
    }
    // increment the number of inputs
    string numberOfInputsStr =
        _getProp(neuralInputsNode, string("numberOfInputs"));
    int numberOfInputs;
    istringstream(numberOfInputsStr) >> numberOfInputs;
    numberOfInputs++;
    stringstream ss;
    ss << numberOfInputs;
    xmlSetProp(neuralInputsNode, (const xmlChar *)"numberOfInputs",
               (const xmlChar *)(ss.str().c_str()));
    // then append the node and its children
    xmlNodePtr neuralInputNode =
        xmlNewChild(neuralInputsNode, 0, (const xmlChar *)"NeuralInput", 0);
    ss.str("");
    ss << id;
    xmlNewProp(neuralInputNode, (const xmlChar *)"id",
               (const xmlChar *)(ss.str().c_str()));

    xmlNodePtr derivedFieldNode =
        xmlNewChild(neuralInputNode, 0, (const xmlChar *)"DerivedField", 0);
    xmlNewProp(derivedFieldNode, (const xmlChar *)"optype",
               (const xmlChar *)(optype.c_str()));
    xmlNewProp(derivedFieldNode, (const xmlChar *)"dataType",
               (const xmlChar *)(dataType.c_str()));

    xmlNodePtr normcontNode =
        xmlNewChild(derivedFieldNode, 0, (const xmlChar *)"NormContinuous", 0);
    xmlNewProp(normcontNode, (const xmlChar *)"field",
               (const xmlChar *)(inputName.c_str()));

    xmlNodePtr node_linearnorm1 =
        xmlNewChild(normcontNode, 0, (const xmlChar *)"LinearNorm", 0);
    ss.str("");
    ss << scientific << orig1;
    xmlNewProp(node_linearnorm1, (const xmlChar *)"orig",
               (const xmlChar *)(ss.str().c_str()));
    ss.str("");
    ss << scientific << norm1;
    xmlNewProp(node_linearnorm1, (const xmlChar *)"norm",
               (const xmlChar *)(ss.str().c_str()));
    xmlNodePtr node_linearnorm2 =
        xmlNewChild(normcontNode, 0, (const xmlChar *)"LinearNorm", 0);
    ss.str("");
    ss << scientific << orig2;
    xmlNewProp(node_linearnorm2, (const xmlChar *)"orig",
               (const xmlChar *)(ss.str().c_str()));
    ss.str("");
    ss << scientific << norm2;
    xmlNewProp(node_linearnorm2, (const xmlChar *)"norm",
               (const xmlChar *)(ss.str().c_str()));
}
...
void PMMLlib::AddNeuralOutput(int outputNeuron, std::string outputName,
    ...
    if (!neuralOutputsNode) {
        neuralOutputsNode =
            xmlNewChild(netNode, 0, (const xmlChar *)"NeuralOutputs", 0);
        xmlNewProp(neuralOutputsNode, (const xmlChar *)"numberOfOutputs",
                   (const xmlChar *)"0");
    }
    // increment the number of inputs
    string numberOfOutputsStr =
        _getProp(neuralOutputsNode, string("numberOfOutputs"));
    int numberOfOutputs;
    istringstream(numberOfOutputsStr) >> numberOfOutputs;
    numberOfOutputs++;
    stringstream ss;
    ss << numberOfOutputs;
    xmlSetProp(neuralOutputsNode, (const xmlChar *)"numberOfOutputs",
               (const xmlChar *)(ss.str().c_str()));

    // then append the node and its children
    xmlNodePtr neuralOutputNode =
        xmlNewChild(neuralOutputsNode, 0, (const xmlChar *)"NeuralOutput", 0);
    ss.str("");
    ss << outputNeuron;
    xmlNewProp(neuralOutputNode, (const xmlChar *)"outputNeuron",
               (const xmlChar *)(ss.str().c_str()));

    xmlNodePtr derivedFieldNode =
        xmlNewChild(neuralOutputNode, 0, (const xmlChar *)"DerivedField", 0);
    xmlNewProp(derivedFieldNode, (const xmlChar *)"optype",
               (const xmlChar *)(optype.c_str()));
    xmlNewProp(derivedFieldNode, (const xmlChar *)"dataType",
               (const xmlChar *)(dataType.c_str()));
...
void PMMLlib::AddNeuralLayer(PMMLActivationFunction activationFunction) {
    ...
    string numberOfLayersStr =
        _getProp(_currentModelNode, string("numberOfLayers"));
    int numberOfLayers;
    istringstream(numberOfLayersStr) >> numberOfLayers;
    numberOfLayers++;
    stringstream ss;
    ss << numberOfLayers;
    xmlSetProp(netNode, (const xmlChar *)"numberOfLayers",
               (const xmlChar *)(ss.str().c_str()));
    // Add the neural layer node
    xmlNodePtr neuralLayerNode =
        xmlNewChild(netNode, 0, (const xmlChar *)"NeuralLayer", 0);
    xmlNewProp(neuralLayerNode, (const xmlChar *)"activationFunction",
               (const xmlChar *)(functionName.c_str()));
    xmlNewProp(neuralLayerNode, (const xmlChar *)"numberOfNeurons",
               (const xmlChar *)"0");
...
void PMMLlib::AddNeuron(int id, double bias, int conNb, int firstFrom,
    ...
    string numberOfNeuronsStr =
        _getProp(_currentNode, string("numberOfNeurons"));
    int numberOfNeurons;
    istringstream(numberOfNeuronsStr) >> numberOfNeurons;
    numberOfNeurons++;
    ss << numberOfNeurons;
    xmlSetProp(_currentNode, (const xmlChar *)"numberOfNeurons",
               (const xmlChar *)(ss.str().c_str()));

    // append a neuron
    xmlNodePtr neuronNode =
        xmlNewChild(_currentNode, 0, (const xmlChar *)"Neuron", 0);
    ss.str("");
    ss << id;
    xmlNewProp(neuronNode, (const xmlChar *)"id",
               (const xmlChar *)(ss.str().c_str()));
    ss.str("");
    ss << scientific << bias;
    xmlNewProp(neuronNode, (const xmlChar *)"bias",
               (const xmlChar *)(ss.str().c_str()));

    // append multiple 'Con' to the neuron
    for (int k = 0; k < conNb; k++) {
        xmlNodePtr conNode =
            xmlNewChild(neuronNode, 0, (const xmlChar *)"Con", 0);
        ss.str("");
        ss << firstFrom + k;
        xmlNewProp(
            conNode, (const xmlChar *)"from",
            (const xmlChar *)(ss.str().c_str())); // !!! ce n'est pas k !!!
        ss.str("");
        ss << scientific << weights[k];
        xmlNewProp(conNode, (const xmlChar *)"weight",
                   (const xmlChar *)(ss.str().c_str()));
    }
}
...
void PMMLlib::fillVectorsForExport(int nInput, int nOutput, int nHidden,
    ...
    for (int j = 0; j < nHidden; j++) // hidden layers
    {
        ...
        for (int i = 0; i < nInput; i++) {
            valW[j * (nInput + nOutput + 1) + 3 + i] =
                GetPrecNeuronSynapse(0, j, i);
        }
    }
    for (int j = 0; j < nOutput; j++) // output layers
    {
        ...
        for (int i = 0; i < nHidden; i++) {
            valW[i * (nInput + nOutput + 1) + 1] =
                GetPrecNeuronSynapse(1, j, i);
        }
    }
}
 ...
 * @param header Header of the function
 */
void PMMLlib::ExportNeuralNetworkCpp(std::string file, std::string functionName,
                                     std::string header) {
    CheckNeuralNetwork();
    ...
    if (normType == 0) { // kMinusOneOne
        ...
        sourcefile << "#define ActivationFunction(sum) ( 1.0 / ( 1.0 + exp( "
                      "-1.0 * sum )) )"
                   << endl;
    }
    //
    sourcefile << "void " << functionName << "(double *param, double *res)"
               << endl;
    sourcefile << "{" << endl;
    // header
    sourcefile << "  ////////////////////////////// " << endl;
    ...
    if (normType == 0) { // kMinusOneOne
        sourcefile << "     " << functionName
                   << "_act[indNeurone++] = 2.0 * ( param[i] - " << functionName
                   << "_minInput[i] ) / ( " << functionName << "_maxInput[i] - "
                   << functionName << "_minInput[i] ) - 1.0;" << endl;
    } else { //  kCR, kZeroOne
        sourcefile << "     " << functionName
                   << "_act[indNeurone++] = ( param[i] - " << functionName
                   << "_minInput[i] ) / " << functionName << "_maxInput[i];"
                   << endl;
    }
    sourcefile << "  }" << endl;
    ...
    sourcefile << "  for (int member = 0; member < nHidden; member++) {"
               << endl;
    sourcefile << "     int CrtW = member * ( nInput + 2) + 2;" << endl;
    sourcefile << "     sum = " << functionName << "_valW[CrtW++];" << endl;
    sourcefile << "     for (int source = 0; source < nInput; source++) {"
               << endl;
    sourcefile << "         sum += " << functionName << "_act[source] * "
               << functionName << "_valW[CrtW++];" << endl;
    sourcefile << "       }" << endl;
    sourcefile << "       " << functionName
    ...
    sourcefile << "  for (int member = 0; member < nOutput; member++) {"
               << endl;
    sourcefile << "    sum = " << functionName << "_valW[0];" << endl;
    sourcefile << "    for (int source = 0; source < nHidden; source++) {"
               << endl;
    sourcefile << "      CrtW = source * ( nInput + 2) + 1;" << endl;
    sourcefile << "      sum += " << functionName << "_act[nInput+source] * "
               << functionName << "_valW[CrtW];" << endl;
    sourcefile << "    }" << endl;
...
void PMMLlib::ExportNeuralNetworkFortran(std::string file,
    ...
    for (int i = 0; i < GetNbInputs(); i++) {
        ...
        if (normType == 0) { // kMinusOneOne
            sourcefile << "2.D0 * ( " << GetNameInput(i) << " - " << minInput[i]
                       << "D0 ) / " << maxInput[i] - minInput[i] << "D0 - 1.0"
                       << endl;
        } else { //  kCR, kZeroOne
            sourcefile << "( " << GetNameInput(i) << " - " << minInput[i]
                       << "D0 ) / " << maxInput[i] << "D0" << endl;
        }
    }
    ...
    for (int member = 0; member < nHidden; member++) {
        ...
        for (int source = 0; source < nInput; source++) {
            sourcefile << "     1      + VW" << CrtW++ << " * VXN"
                       << GetNameInput(source) << endl;
        }
        sourcefile << endl;

        if (normType == 0) { // kMinusOneOne
            sourcefile << "      VPot" << member + 1
    ...
    for (int source = 0; source < nHidden; source++) {
        ...
        sourcefile << "     1    + VW" << CrtW << " * VPot" << source + 1
                   << endl;
    }
    ...
    if (normType == 0) { // kMinusOneOne
        sourcefile << "      VDelta = " << 0.5 * (maxOutput[0] - minOutput[0])
                   << "D0 * ( VOut + 1.0D0)" << endl;
        sourcefile << "      " << GetNameOutput(0) << " = " << minOutput[0]
                   << "D0 + VDelta" << endl;

    } else { //  kCR, kZeroOne
        sourcefile << "      " << GetNameOutput(0) << " = " << minOutput[0]
                   << "D0 + " << maxOutput[0] << "D0 * VOut;" << endl;
    }
...
std::string PMMLlib::ExportNeuralNetworkPyStr(std::string functionName,
    ...
    if (normType == 0) { // kMinusOneOne
        out << "        " << functionName << "_act.append( 2.0 * ( param[i] - "
            << functionName << "_minInput[i] ) / ( " << functionName
            << "_maxInput[i] - " << functionName << "_minInput[i] ) - 1.0 ) ;"
            << endl;
    } else { //  kCR, kZeroOne
    ...
    out << "            sum += " << functionName << "_act[source] * "
        << functionName << "_valW[CrtW];" << endl;
    out << "            CrtW += 1 ;" << endl;
    out << "            pass" << endl;
    out << "        " << functionName
    ...
    out << "            sum += " << functionName << "_act[nInput+source] * "
        << functionName << "_valW[CrtW];" << endl;
    out << "            pass" << endl;
    ...
    if (normType == 0) { // kMinusOneOne
        ...
        out << "        res.append( " << functionName << "_minOutput[member] + "
            << functionName << "_maxOutput[member] * sum );" << endl;
    }
    out << "        pass" << endl;
    out << endl;
...
void PMMLlib::AddRegressionModel(std::string modelName,
    ...
    xmlNodePtr netNode =
        xmlNewChild(_rootNode, 0, (const xmlChar *)"RegressionModel", 0);
    xmlNewProp(netNode, (const xmlChar *)"functionName",
               (const xmlChar *)(function.c_str()));
    xmlNewProp(netNode, (const xmlChar *)"modelName",
               (const xmlChar *)(_currentModelName.c_str()));
    xmlNewProp(netNode, (const xmlChar *)"targetFieldName",
...
void PMMLlib::AddRegressionTable(double intercept) {
    ...
    xmlNodePtr tableNode = xmlNewChild(_currentModelNode, 0,
                                       (const xmlChar *)"RegressionTable", 0);
    if (intercept != 0.0) {
        ss << scientific << intercept;
        xmlNewProp(tableNode, (const xmlChar *)"intercept",
                   (const xmlChar *)(ss.str().c_str()));
    }
    _currentNode = tableNode;
}
 ...
 */
void PMMLlib::AddNumericPredictor(std::string neuronName, int exponent,
                                  double coefficient) {
    CheckRegression();
    stringstream ss;
    xmlNodePtr numPrecNode =
        xmlNewChild(_currentNode, 0, (const xmlChar *)"NumericPredictor", 0);
    xmlNewProp(numPrecNode, (const xmlChar *)"name",
               (const xmlChar *)(neuronName.c_str()));
    ss.str("");
    ss << exponent;
    xmlNewProp(numPrecNode, (const xmlChar *)"exponent",
               (const xmlChar *)(ss.str().c_str()));
    ss.str("");
    ss << scientific << coefficient;
    xmlNewProp(numPrecNode, (const xmlChar *)"coefficient",
               (const xmlChar *)(ss.str().c_str()));
}
 ...
 * @brief Specific to RegressionModel
 * @param coefficient Value of property "coefficient"
 * @param fieldRef List of values for property "field", one per FieldRef to add
 * to the PredictorTerm
 */
void PMMLlib::AddPredictorTerm(double coefficient,
                               std::vector<std::string> fieldRef) {
    CheckRegression();
    stringstream ss;
    xmlNodePtr predTermNode =
        xmlNewChild(_currentNode, 0, (const xmlChar *)"PredictorTerm", 0);
    ss.str("");
    ss << scientific << coefficient;
    xmlNewProp(predTermNode, (const xmlChar *)"coefficient",
               (const xmlChar *)(ss.str().c_str()));
    vector<string>::iterator it;
    for (it = fieldRef.begin(); it != fieldRef.end(); it++) {
        xmlNodePtr fieldRefNode =
            xmlNewChild(predTermNode, 0, (const xmlChar *)"FieldRef", 0);
        ss.str("");
        ss << (*it);
        xmlNewProp(fieldRefNode, (const xmlChar *)"field",
                   (const xmlChar *)(ss.str().c_str()));
    }
}
...
std::string PMMLlib::GetNumericPredictorName(int num_pred_index) {
    CheckRegression();
    string strName("");
    xmlNodePtr numPredNodes =
        GetChildByName(_currentModelNode, "RegressionTable");
...
/**
 * Get the name of the field_index-th FieldRef for the pred_term_index-th
 * predictor term.
 ...
 */
std::string PMMLlib::GetPredictorTermFieldRefName(int pred_term_index,
                                                  int field_index) {
    CheckRegression();
...
void PMMLlib::ExportLinearRegressionCpp(std::string file,
    ...
    for (int i = 0; i < nPred; i++) {
        ...
        exportfile << "  y += param[" << i << "]*"
                   << GetNumericPredictorCoefficient(i) << ";";
        exportfile << endl << endl;
    }
    nPred = GetPredictorTermNb();
    for (int i = 0; i < nPred; i++) {
        exportfile << "  // Attribute : " << GetPredictorTermName(i) << endl;
        exportfile << "  y += param[" << (i + nPred) << "]*"
                   << GetPredictorTermCoefficient(i) << ";";
        exportfile << endl << endl;
    }
...
void PMMLlib::ExportLinearRegressionFortran(std::string file,
    ...
    for (int i = 0; i < nNumPred; i++) {
        exportfile << "C --- Attribute : " << GetNumericPredictorName(i)
                   << endl;
        exportfile << "      Y += P[" << i << "]*"
                   << GetNumericPredictorCoefficient(i) << ";";
        exportfile << endl << endl;
